...
def get_ai_insights(cursor) -> List[Dict[str, Any]]:
...
def _analyze_budget_trends(cursor) -> Dict[str, Any]:
    cursor.execute('''
...
def _analyze_delay_patterns(cursor) -> Dict[str, Any]:
    cursor.execute('''
        ...
        GROUP BY surtax_category HAVING COUNT(*) >= 2
        ORDER BY (SUM(CASE WHEN is_delayed = 1 THEN 1 ELSE 0 END) * 1.0 / COUNT(*)) DESC LIMIT 1
    ''')
    row = cursor.fetchone()
...
def _analyze_vendor_performance(cursor) -> Dict[str, Any]:
    cursor.execute('''
        ...
        GROUP BY vendor_name HAVING COUNT(*) >= 2
        ORDER BY (AVG(CASE WHEN is_delayed = 1 THEN 1.0 ELSE 0.0 END) + AVG(CASE WHEN is_over_budget = 1 THEN 1.0 ELSE 0.0 END)) DESC LIMIT 1
    ''')
    row = cursor.fetchone()
...
def _analyze_spending_efficiency(cursor) -> Dict[str, Any]:
    cursor.execute('''
